                        ...
                        <tbody>
                            @foreach ($query as $data)
                                <tr>
                                    ...
                                    <td>
                                        <input type="checkbox" @if ($data->is_active === "1" || $data->is_active === 1) checked @endif disabled />
                                    </td>
                                    <td>
                                        {{ $data->updated_at }}
                                    </td>
                                    <td class="d-print-none">
                                        @if ($provider::access('modules')->access[0]->can_update == 1)
                                            <a href="{{ route('admin.modules.edit', $data->id) }}">
                                                <span class="badge bg-warning p-1"><i class="fa fa-edit"></i> Edit</span>
                                            </a>
                                        @endif

                                        @if ($provider::access('modules')->access[0]->can_delete == 1)
...
@section('page-script')
    ...
    <script>
        ...
        function create() {
            var can_access = {!! json_encode($provider::access('modules')->access[0]->can_create) !!};
            if (can_access == 0) {
                alert("Access denied");
                return;
            }
