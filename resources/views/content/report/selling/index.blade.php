    ...
    <script>
        ...
        function show() {
            ...
            $.ajax({
                dataType: 'json',
                type: "GET",
                url: '{{ env('APP_URL') }}' + "/api/report/selling/" + document.getElementById('date_from').value + "/" + document.getElementById('type').value,
                    ...
                    if (res.length > 0) {  
                        for (let index = 0; index < res.length; index++) {     
                            ...
                            for (let col = 1; col <= 31; col++) {  
                                sSelling = formatNumber(res[index]['sell_date' + col.toString().padStart(2, '0')], 0);
                                if (sSelling == 0) { sSelling = ""; }   
                                ...
                                if (document.getElementById('type').value == 0) {
                                    html += 
                                        '<td class="text-right">' + sSelling + '</td>'
                                    ;                                     
                                }
