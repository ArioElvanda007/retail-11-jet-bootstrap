                                ...
                                <tr>
                                    ...
                                    <td class="d-print-none">
                                        @if ($provider::access('banks')->access[0]->can_update == 1)
                                            <a href="{{ route('accounting.banks.edit', $data->id) }}">
                                                <span class="badge bg-warning p-1"><i class="fa fa-edit"></i> Edit</span>
                                            </a>
                                        @endif

                                        @if ($provider::access('banks')->access[0]->can_delete == 1)
                                            <a onclick="return confirm('Are you sure?')"
                                                href="{{ route('accounting.banks.destroy', $data->id) }}">
...
@section('page-script')
    ...
    <script>
        ...
        function create() {
            var can_access = {!! json_encode($provider::access('banks')->access[0]->can_create) !!};
            if (can_access == 0) {
                alert("Access denied");
                return;
            }

            window.location.href = "{{ route('accounting.banks.create') }}";
        }
    </script>
@endsection
